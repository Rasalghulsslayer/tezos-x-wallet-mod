...
This wallet targets the **Tezos X testnet** exclusively. The UI shows a persistent testnet badge. RPC endpoints, contract addresses, and chain IDs are hardcoded to testnet values in `packages/wallet/src/lib/constants.ts`. Do not attempt to use this wallet on Tezos mainnet or Etherlink mainnet.
