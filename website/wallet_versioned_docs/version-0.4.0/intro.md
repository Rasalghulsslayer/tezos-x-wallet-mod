...
- **XTZ to a Tezos address** (`tz1 → tz1 / tz2 / tz3 / KT1`) — emitted as a **native Tezos L1 operation** via Taquito, no NAC gateway involved.
...
- **USDC and any dApp-initiated EVM call** — routed through the NAC gateway's `call_evm` entrypoint, executed atomically by the Etherlink kernel.
