...
|---|---|---|
| `tz1…` | tz1VSUr8wwNhLAzempoch5d6hLRiTh8Cjcjb | Same-runtime · Tezos L1 (native) |
| `tz2…` | tz2TSvNTh2epDMhZHrw73nV9piBX7kLZ9K9m | Same-runtime · Tezos L1 (native) |
| `tz3…` | tz3Nk25mfsfsceBdHXDqHDP6KpUNAbGxksZX | Same-runtime · Tezos L1 (native) |
| `KT1…` | KT18oDJJKXMKhfE1bSuAPGp92pYcwVDiqsPw  | Same-runtime · Tezos L1 (native, `default unit`) |
...
### Same-runtime: native Tezos L1 transfer

For any `tz1 / tz2 / tz3 / KT1` recipient, the wallet emits a **plain Tezos L1 transfer** with no contract call:
...
No NAC gateway, no synthetic EVM hash, no block scanning. The hash returned to the popup is the **Tezos L1 op hash** (`o…`, Base58Check, ~51 chars), browsable on tzkt.
...
| Path | Hash format | Explorer |
|---|---|---|
| Same-runtime native | `o…` Base58 (~51 chars) | tzkt |
| Cross-runtime via gateway | `0x…` 32-byte hex | Blockscout (resolved real EVM hash) |
